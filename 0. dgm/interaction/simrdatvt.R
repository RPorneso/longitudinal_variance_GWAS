simrdatvt <- function(nsnp, N, t, TIME, Lambda, maf, bsnp){
  ...
  # generated by modeling snp-by-time interaction, where the snp is of a given    
  # maf, to have an interaction effect + an error term. Qt is correlated by taking the  
  ...
  Y0 = snp %*% matrix(TIME, 1, t) * bsnp + matrix(rnorm(N * t), N, t)
  Y = snp %*% matrix(TIME, 1, t) * bsnp + emat # %*% operator promotes snp vec into a matrix
  
  dat <<- data.frame(qt = c(Y), phe = c(Y0), snp = rep(snp, times = t), 
                     time = rep(TIME, each = N), 
