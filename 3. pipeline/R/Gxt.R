...
inputfile <- args[4]
load(inputfile)
# df <- eval(parse(text = ls()))                                   # REMOVE FOR HEIGHT AND BMI
# df$time <- ifelse(df$LEVEL=="5", 0, ifelse(df$LEVEL=="8", 3,4))  # REMOVE FOR ALL EXCEPT READ
# rm(list=setdiff(ls(),"df"))                                      # REMOVE FOR HEIGHT AND BMI
print(dim(df))
...
# For each SNP, subset .bed to those with phenotypes, calculate MAF, standardize SNP by MAF, repeat variant t times, and remove NAs while creating temp df, dfa; run LME  on dfa
...
runmods <- function (i) {
  ...
  zSNP <- (SNP - 2*maf) / sqrt(2*maf*(1-maf))

  repzSNP <- rep(zSNP, each = 3)
  indna <- is.na(repzSNP)
  dfa <- df[!indna,]
  dfa$SNP <- repzSNP[!indna]
  
 mod <- lme(resZ_SCORE ~ time + Sex + genotyping_batch_num + SNP + SNP*time + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = dfa, random = list(IID = ~1), method = "ML", control = lmeControl(opt="optim")) # EXCL AARGANG, CHANGE sex to Sex for ANTHRO
  
  c(round(maf,2), summary(mod)$tTable[[5]], summary(mod)$tTable[[21]], summary(mod)$tTable[[69]], summary(mod)$tTable[[16]], summary(mod)$tTable[[32]], summary(mod)$tTable[[80]], nrow(dfa)) # UPDATE COEF INDICES FOR ANTHRO
}
...
# Create results table using plink$map and calculate MAF using sub-sample
...
colnames(temp) <- c("maf", "beta", "se", "p", "beta2", "se2", "p2", "n")
