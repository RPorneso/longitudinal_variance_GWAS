...
runmods <- function (sigsnps) {
  
  SNP <- plink$genotypes[,sigsnps]
  ...
  mod0 <- lme(resZ_SCORE ~ AARGANG + time + sex + genotyping_batch_num + SNP + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = dfa, random = list(IID = ~1), method = "ML", control = lmeControl(opt="optim")) # base model SHOULD INCL AARGANG FOR READ/MATH BUT SHOULD BE REMOVED FOR LOGBMI/HEIGHT
  mod <- lme(resZ_SCORE ~ AARGANG + time + sex + genotyping_batch_num + SNP + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = dfa, random = list(IID = ~1), weights = varExp(form = ~SNP), method = "ML", control = lmeControl(opt="optim")) # SHOULD INCLUDE AARGANG FOR READ/MATH BUT SHOULD BE REMOVED FOR LOGBMI/HEIGHT
  lrt <- anova(mod0, mod)
  
  c(round(maf,2), summary(mod)$tTable[[6]], summary(mod)$tTable[[22]], summary(mod)$tTable[[70]], lrt[[9]][[2]], nrow(dfa), summary(mod)$modelStruct$varStruct[[1]]) # 5, 20, 65 NEED TO CHANGE TO 6, 22, 70 BETWEEN HEIGHT/LOGBMI AND READ/MATH
}

estimates <- list()
estimates <- lapply(sigsnps, function(i) tryCatch(runmods(i), error=function(e) NULL))
...
results <- plink$map[unlist(sigsnps),]
